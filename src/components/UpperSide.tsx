    ...
    <div className="flex flex-row">
      ...
      <div className="flex absolute flex-row border-gray-300 text-white ">
        {tabsFile.map((tab ,id) => (
          <div
            key={id}
            className={`flex flex-row items-center justify-between p-2 text-sm hover:bg-zinc-800 cursor-pointer border-r-[1px] border-gray-300 w-48 font-bold text-md duration-300`}
            style={openFile === tab ? { borderBottom: "1px solid black"} : { borderBottom: "1px solid #d1d5db"}}
            onClick={() => dispatch(changeTab(tab))}
          >
            <div className="flex flex-row">
                <SVG type={tab.split('.').length > 2 ? tab.split('.')[0] : tab.split('.')[1]} isFolder={false} />
                {tab}
            </div>
